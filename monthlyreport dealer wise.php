...
<div class="table-responsive">
   <table  id="demo" class="table table-hover" >
      ...
      <?php 
         ...
         $ss[]=$row["District"]; }
         $sqlss = "SELECT DISTINCT Dealer FROM sales UNION SELECT DISTINCT Dealer FROM Salesmaster ORDER by Dealer ASC";
      ...
      <tr>
         ...
         <?php
            ...
            { 
            $sqldo = "SELECT sum(QTY),sum(Amount) from Salesmaster WHERE Dealer='$di[$j]' AND District='$ss[$i]' ".$queryCondition1;
                       $resultdo = mysqli_query($conn,$sqldo);     
                       $rowdo= mysqli_fetch_array($resultdo);
                        $sqld1 = "SELECT sum(QTY),sum(Amount) from sales WHERE Dealer='$di[$j]' AND District='$ss[$i]' ".$queryCondition;
                        //echo $sqld1;
