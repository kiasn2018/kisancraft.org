   ...
   <table  id="testTable" class="table table-hover"   >
      ...
      <tr >
         <td bgcolor="#aca9ce" style=" border: 0.5pt solid #000000; "><?php
            ...
            ?></td>
         <td bgcolor="#aca9ce" style=" border: 0.5pt solid #000000; "><?php
            echo  number_format($tpa1[$i],0); 
            ?></td>
         <td bgcolor="#aca9ce" style=" border: 0.5pt solid #000000; "><?php
            echo $tpd[$i];
            ?></td>
         <td bgcolor="#aca9ce" style=" border: 0.5pt solid #000000; "><?php
            echo number_format($tpa[$i],0);
            ?></td>
      </tr>
      <?php } ?>
      <tr></tr>
         ...
         $disvp=array();
            $sqle   = " SELECT distinct Executive, sum(Amount) from sales where Zone='$state' ".$queryCondition." group by Executive ";
                ...
                $sqlsmp   = " SELECT distinct SM, sum(Amount) from Salesmaster where Zone='$state' ".$queryCondition1." group by SM ";
                 ...
                 $sqlseg   = " SELECT  sum(QTY),sum(Amount) from sales where Zone='$state' AND supersegment='$m' ".$queryCondition;
                 //$sqld = "SELECT DISTINCT (D_distict) D_distict, D_state FROM Dealermst";
                  //echo $sqlseg;
                 ...
                 while ($rowseg = mysqli_fetch_array($resultseg)) {
                   ...
                   $seg[]=$m;
                   $segm[]=$rowseg["sum(QTY)"];
                   $segv[]=$rowseg["sum(Amount)"];
                 }
                  $sqlsegp   = " SELECT  sum(QTY),sum(Amount) from Salesmaster where Zone='$state' AND supersegment='$m' ".$queryCondition1;
                 //$sqld = "SELECT DISTINCT (D_distict) D_distict, D_state FROM Dealermst";
                 // echo $sqlsegp;
                 ...
                 while ($rowsegp = mysqli_fetch_array($resultsegp)) {
                   ...
                   $segvp[]=$rowsegp["sum(Amount)"];
                 }}
         
                 //district wise
                 //segment wise details
                  $sqlseg1   = " SELECT DISTINCT District FROM sales where Zone='$state' UNION SELECT DISTINCT District  FROM Salesmaster where Zone='$state' ORDER by District ASC";
                 //$sqld = "SELECT DISTINCT (D_distict) D_distict, D_state FROM Dealermst";
                  //echo $sqlseg;
                 $resultseg1 = mysqli_query($conn, $sqlseg1);
                 while ($rowseg1 = mysqli_fetch_array($resultseg1)) {
                 ...
                 $sqlseg   = " SELECT  sum(Amount) from sales where Zone='$state' AND District='$k' ".$queryCondition;
                 //$sqld = "SELECT DISTINCT (D_distict) D_distict, D_state FROM Dealermst";
                  //echo $sqlseg;
                 $resultseg = mysqli_query($conn, $sqlseg);
                 while ($rowseg = mysqli_fetch_array($resultseg)) {
                   ...
                   $disv[]=$rowseg["sum(Amount)"];
                 }
                  $sqlsegp   = " SELECT  sum(Amount) from Salesmaster where Zone='$state' AND District='$k' ".$queryCondition1;
                 //$sqld = "SELECT DISTINCT (D_distict) D_distict, D_state FROM Dealermst";
                 // echo $sqlsegp;
                 $resultsegp = mysqli_query($conn, $sqlsegp);
                 while ($rowsegp = mysqli_fetch_array($resultsegp)) {
      ...
      <tr>
         ...
         <th bgcolor="#a9c1e8" style=" border: 0.5pt solid #000000; ">17-18 Value</th>
         <th bgcolor="#a9c1e8" style=" border: 0.5pt solid #000000; ">18-19 QTY</th>
         <th bgcolor="#a9c1e8" style=" border: 0.5pt solid #000000; ">18-19 Value</th>
         <th bgcolor="#a9c1e8" style=" border: 0.5pt solid #000000; "> % of Progress</th>
      </tr>
      ...
      <tr></tr>
      <tr>
         <th bgcolor="#a9c1e8" style=" border: 0.5pt solid #000000; ">District</th>
         <th bgcolor="#a9c1e8" style=" border: 0.5pt solid #000000; ">17-18 Value</th>
         <th bgcolor="#a9c1e8" style=" border: 0.5pt solid #000000; ">18-19 Value</th>
         <th bgcolor="#a9c1e8" style=" border: 0.5pt solid #000000; "> % of Progress</th>
      </tr>
