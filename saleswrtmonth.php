...
<?php 
   ...
   if($_POST["go"]=="Submit"){
      ...
      {   $seg=array();
	        ...
	        if($state1=='ALL'){$sqld = "SELECT DISTINCT State from sales where Date!='0000-00-00' ";}else{
      ...
      <tr>
         ...
         <?php 
            $sqldz = "SELECT Zone from Zonesmst WHERE District='$di'  ";
			   ...
			   if($state1=='ALL'){ $sqld1p = "SELECT sum(Amount),Executive,State from sales WHERE State='$state'  ".$queryCondition2; }else{
               ...
               $rowd1p= mysqli_fetch_array($resultd1p);	
			    if($state1=='ALL'){  $sqlddp = "SELECT sum(Amount),Executive,State From Salesmaster WHERE State='$state'  ".$queryCondition3;}else{
