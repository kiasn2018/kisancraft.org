...
?>
     <div class="main-panel">
      <div class="content-wrapper">
         <div class ="row">
            <div class="col-6" >
               <h1 style="text-align:center;"> Weekly Report Executive wise </h1>
                           ...
                           <div class="form-group row">
                              ...
                              <div class="col-sm-9">
                                 <select onChange="getdistrict(this.value);"  name="state" id="state" class="form-control" >
                                    ...
                                    <?php
$sqlst    = "SELECT Distinct emp,category FROM `Excutives` ";
$resultst = mysqli_query($conn, $sqlst);

while ($rowst = mysqli_fetch_array($resultst)) {
?>
                                   <option value="<?php
    echo $rowst['emp']."-".$rowst['category'];
                                       ?>"><?php
                                       echo $rowst['emp']."-".$rowst['category'];
?></option>
                                    <?php
...
if (!isset($_POST["go"])) {
?>
              <table  id="testTable" class="sortable">
                  <thead>
                     <tr class="d0">
...
if ($_POST["go"] == "Submit") {
    ...
    //echo 
    {
        ...
        $amto = array();
            ...
            IF( $des == "Executive"){
                     $sqld = "SELECT DISTINCT District,Zone,Executive,ASM,SM,ZM FROM sales where Executive='$emp' UNION SELECT DISTINCT District,Zone,Executive,ASM,SM,ZM  FROM Salesmaster where Executive='$emp' ORDER by Zone,Executive,ASM,SM,ZM,District ASC";
                     $sqlc     = "SELECT count(DISTINCT Executive),count(DISTINCT ASM) FROM sales where Executive='$emp' UNION SELECT count(DISTINCT Executive),count(DISTINCT ASM) FROM Salesmaster where Executive='$emp' ";

                  }elseif($des == "ASM")
                  {
                     $sqld = "SELECT DISTINCT District,Zone,Executive,ASM,SM,ZM FROM sales where ASM='$emp' UNION SELECT DISTINCT District,Zone,Executive,ASM,SM,ZM  FROM Salesmaster where ASM='$emp' ORDER by Zone,Executive,ASM,SM,ZM,District ASC";
                     $sqlc     = "SELECT count(DISTINCT Executive),count(DISTINCT ASM) FROM sales where ASM='$emp' UNION SELECT count(DISTINCT Executive),count(DISTINCT ASM) FROM Salesmaster where ASM='$emp' ";
                  }elseif($des == "SM")
                  {
                  $sqld = "SELECT DISTINCT District,Zone,Executive,ASM,SM,ZM FROM sales where SM='$emp' UNION SELECT DISTINCT District,Zone,Executive,ASM,SM,ZM  FROM Salesmaster where SM='$emp' ORDER by Zone,Executive,ASM,SM,ZM,District ASC";
                   $sqlc     = "SELECT count(DISTINCT Executive),count(DISTINCT ASM) FROM sales where SM='$emp' UNION SELECT count(DISTINCT Executive),count(DISTINCT ASM) FROM Salesmaster where SM='$emp' ";
                  }elseif($des == "ZM")
                  {
                     $sqld = "SELECT DISTINCT District,Zone,Executive,ASM,SM,ZM FROM sales where ZM='$emp' UNION SELECT DISTINCT District,Zone,Executive,ASM,SM,ZM  FROM Salesmaster where ZM='$emp' ORDER by Zone,Executive,ASM,SM,ZM,District ASC";
                     $sqlc     = "SELECT count(DISTINCT Executive),count(DISTINCT ASM) FROM sales where ZM='$emp' UNION SELECT count(DISTINCT Executive),count(DISTINCT ASM) FROM Salesmaster where ZM='$emp' ";
                  }else{
                    echo "Non";
                  }
...
?></h1>
   <table  id="testTable" class="table table-hover" >
     <tr>
     <th style=" border: 0.5pt solid #000000; ">From Date</th>  <td style=" border: 0.5pt solid #000000; "><?php echo $post_at; ?></td>
     <th style=" border: 0.5pt solid #000000; ">To Date</th>  <td style=" border: 0.5pt solid #000000; "><?php  echo $post_at_todate; ?></td>
     </tr>
      <tr>
...
?></td>
   ...
   <input type="button" onclick="tableToExcel('testTable', '<?php echo $emp; ?>')" value="Export to Excel">
</div>
